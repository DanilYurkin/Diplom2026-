...
def init_db():
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        ...
        # Таблица товаров (единое определение с btb_price)
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS products (
            ...
            offer_id TEXT,
            name TEXT,
            price REAL,        -- Ozon цена
            old_price REAL,
            btb_price REAL,    -- цена BTB
            currency_code TEXT,
            is_archived BOOLEAN,
...
def save_qr_code(order_id, qr_data, image_path):
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        ...
        ''', (order_id, qr_data, image_path))
        conn.commit()
        print(f"✅ QR-код для заказа {order_id} сохранен в БД")
...
def get_qr_code(order_id):
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        cursor = conn.cursor()
        cursor.execute('''
            SELECT image_path FROM qr_codes WHERE order_id = ?
        ''', (order_id,))
        result = cursor.fetchone()
        return result[0] if result else None


# Функция get_all_products – исправлена: убирает дубликаты складов, но не фильтрует нулевые
def get_all_products():
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        ...
        for row in products:
            ...
            # Убираем дубликаты складов (оставляем только первое вхождение для каждого источника)
            unique_stocks = []
            seen_sources = set()
        ...
        with closing(sqlite3.connect('Ozon_products.db')) as conn:
            ...
            for product in products:
                ...
                ''', (
                    ...
                    product['price'],
                    product['old_price'],
                    None,   # btb_price изначально отсутствует
                ...
                for stock in product.get('stocks', []):
                    cursor.execute('''
                    INSERT INTO stocks (product_id, source, present, reserved)
                    VALUES (?, ?, ?, ?)
...
def update_btb_price(product_id, price):
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        cursor = conn.cursor()
        print(f"[DEBUG] Обновление цены BTB: product_id={product_id}, price={price}")
...
def save_order(order_data):
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        ...
        conn.commit()
    print("Заказ сохранен в базу данных.")
